    ...
    $('#add').click(function () {
       ...
       console.log("Hello world! 1");
        ...
        $('#addDiv').append(
           ...
           "<div class='newService-sub' id='fullSlotDetail" + j + "'> <div class='btn-remove quantity-align'><a href='#fullSlotDetail " + j + "' name='remove' id='" + i + "' class='close-slot'><i class='fas fa-times fa-1g'></i><br /></a></div><h4 class='paddingBottom'>Slot " + i + "</h4><div class='row'><div class='column'>           <div class='row2' id='intervalDetails" + j + "'><label class='labels'>Interval Duration</label><br><select class='dropdownSelectBox     intervalSelectBox" + j + "    '></select><span class='error paddingLeft'></span></div>               <div class='row4' id='slotDetails" + i + "'><label class='labels'>Slot Duration</label><br><select class='dropdownSelectBox        slotDurationSelectBox" + i + "'><option class='unbold' value='val0' option selected='true' disabled='disabled'>Select duration</option><option value='val1'>1 min</option><option value='val2'>2 min</option></select><span class='error paddingLeft'></span></div></div>                     <div class='column' id=''><label class='labels'>Resources and Quantity</label> <div class='checkbox-div      resourceDetails" + i + "'><div class='divIndiv     resourceContentDiv" + i + "     '> <label class='lableInDiv         resourceName" + i + "' id='checkedItem'></label> <select class='dropdownSelectBox-small quantity-align resCount             resourceCountSelectBox" + i + "'></select><br></div><hr class='resHr'></div><span class='error paddingLeft'></span></div></div></div>"
        );
...
function resorcesCountForResource() {
    fetch(`http://localhost:80/beauty-craft/Services/getResourceForSlots`)
      .then(response => response.json())
      .then(sResource => {
        resourceCountSelectDropDown.innerHTML = "";
         ...
         sResource.forEach(sRes => {
             var option = document.createElement("option");
             option.text = "R" + sRes.resourceID + " - " + sRes.name;
             option.value = sProvider.resourceID;
